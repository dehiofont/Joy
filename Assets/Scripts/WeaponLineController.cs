...
{
    [SerializeField] GameManager Mother;
    ...
    public void AttachToFoundEnemy()
    {
        ...
        _enemiesInCombatSphere = Mother.TargetDetector.targetsinCombatSphere;
        currentlySelectedObject = _enemiesInCombatSphere[Mother.PlayerCombatSphereSelectionController.currentSelectedSlot];
    ...
    private void UpdateLineRendererPositions()
    {
        foreach (LineRenderer _weaponLineRenderer in weaponLineRenderers)
        {
            if (_weaponLineRenderer.enabled == true)
            {
                ...
                Vector3 _endPosition = Mother.Player.transform.position;
